...
class LateralController:
    ...
    def stanley(self, waypoints, speed):
        ...
        args:
            ...
            speed (float)
        '''
        Y = waypoints[:, 1] - np.array([48,0])
        X = waypoints[:, 0] - np.array([48,0])

        result = Y - X
        orientation_error = np.arctan(result[0] / result[1])
        ...
        steering_angle = orientation_error + np.arctan(self.gain_constant * crosstrack_error / (speed + 0.00001) )
        ...
        steering_angle = np.clip(steering_angle, -0.4, 0.4) / 0.8
