...
class LaneDetection:
    ...
    def cut_gray(self, state_image_full):
        ...
        '''
        B = state_image_full[:, :, 0] * 0.2989
        G = state_image_full[:, :, 1] * 0.5870
        R = state_image_full[:, :, 2] * 0.1140
        gray_state_image = (R + G + B)
    ...
    def edge_detection(self, gray_image):
        ...
        '''
        x, y = np.gradient(gray_image)

        gradient_sum = np.sqrt(x ** 2 + y ** 2)
    ...
    def find_maxima_gradient_rowwise(self, gradient_sum):
        ...
        for i in range(gradient_sum.shape[0]):
            peaks, _ = find_peaks(gradient_sum[i], distance=self.distance_maxima_gradient)
            for j in peaks:
                tmp.append(np.array([j, i]))

        argmaxima = np.asarray(tmp).T
    ...
    def lane_detection(self, state_image_full):
        ...
        if lane_found:
            ...
            while maxima.shape[1] > 0:
                dist1 = (maxima[0, :] - lane_boundary1_points[-1, 0]) ** 2 + (maxima[1, :] - lane_boundary1_points[-1, 1]) ** 2
                low_edge1 = np.argsort(dist1)[0]

                if dist1[low_edge1] >= 100:
                    break

                lane_boundary1_points = np.append(lane_boundary1_points, [maxima[:, low_edge1]], axis=0)

                if maxima.shape[1] == 0:
                    break

                maxima = np.delete(maxima, low_edge1, axis=1)

            while maxima.shape[1] > 0:
                dist2 = (maxima[0, :] - lane_boundary2_points[-1, 0]) ** 2 + (maxima[1, :] - lane_boundary2_points[-1, 1]) ** 2
                low_edge2 = np.argsort(dist2)[0]

                if dist2[low_edge2] >= 100:
                    break

                lane_boundary2_points = np.append(lane_boundary2_points, [maxima[:, low_edge2]], axis=0)

                if maxima.shape[1] == 0:
                    break

                maxima = np.delete(maxima, low_edge2, axis=1)
