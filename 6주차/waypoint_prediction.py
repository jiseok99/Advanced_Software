...
def curvature(waypoints):
    ...
    for i in range(np.diff(waypoints, axis=1).shape[1] - 1):
...
def waypoint_prediction(roadside1_spline, roadside2_spline, num_waypoints=6, way_type = "smooth"):
    ...
    if way_type == "center":
        ...
        Example)
            ...
            way_points example :
                    array([[47.5, 47.5, 47.5, 47.5, 47.5, 47.5],
                           [ 0. , 12.8, 25.6, 38.4, 51.2, 64. ]])
        '''
        x = 0.5 * (lane_boundary1_points_points[0] + lane_boundary2_points_points[0])
        y = 0.5 * (lane_boundary1_points_points[1] + lane_boundary2_points_points[1])
        
        way_point_center = np.vstack((x, y))
        ...
        way_points_center = np.hstack((x, y))        
        # optimization
        '''
...
def target_speed_prediction(waypoints, num_waypoints_used=5,
    ...
    tmp = np.exp(-exp_constant * abs(num_waypoints_used - 2 - curv_center))
    target_speed = (max_speed - min_speed) * tmp + min_speed
